...
import { ArrowUpRight, ArrowDownRight, Minus, TrendingUp, TrendingDown, Activity, BellRing, Droplets, Moon, ChevronRight, CheckCircle2, ShieldCheck, Info, X, Heart, Wind, Zap } from "lucide-react";
  ...
  const fetchDashboardData = useCallback(async () => {
    ...
    try {
      ...
      const [
        ...
        supabase.from("vitals").select("*").eq("user_id", userId).order("recorded_at", { ascending: false }).limit(20),
      ...
      console.log("Vitals full data:", last2Vitals);
      ...
      const risk = {
        ...
        change: -2, 
        explanation: "Based on your latest vitals and daily logs",
      ...
      const vitalsList = DEFAULT_VITALS.map(v => {
        ...
        if (v.label === "Heart Rate") {
          const hr = Number(latestVitals.heart_rate);
          val = hr || "--";
          ...
          if (allVitalsForTrends && allVitalsForTrends.length > 0) {
            const hrValues = (allVitalsForTrends as any[]).map(r => r.heart_rate).filter(Boolean);
        ...
        if (v.label === "Blood Pressure") {
          const sys = Number(latestVitals.systolic_bp);
          const dia = Number(latestVitals.diastolic_bp);
          val = (sys && dia) ? `${sys}/${dia}` : "--";
          if (sys >= 150 || dia >= 100) status = "critical";
          ...
          if (allVitalsForTrends && allVitalsForTrends.length > 0) {
            const sysValues = (allVitalsForTrends as any[]).map(r => r.systolic_bp).filter(Boolean);
        ...
        if (v.label === "SpO2") {
          const s = Number(latestVitals.spo2);
          val = s || "--";
          if (s < 90) status = "critical";
        ...
        if (v.label === "Weight") {
          const w = Number(latestVitals.weight);
          val = w || "--";
          status = "normal";
        }
      ...
      const moodValue = latestMood ? latestMood.mood.toLowerCase() : "";

      if (moodValue) {
        if (moodValue === "happy" || moodValue === "good") {
      ...
      let moodScore = 0;
      if (moodValue === "happy" || moodValue === "good") moodScore = 100;
