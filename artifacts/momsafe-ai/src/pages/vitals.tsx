...
import { Activity, Download, Loader2, Brain, Sparkles, ChevronUp, Clock, History, CheckCircle2, AlertTriangle, Lightbulb, MessageSquare, Info } from "lucide-react";
...
export default function Vitals() {
  ...
  const fetchVitals = useCallback(async () => {
    ...
    try {
      ...
      const records = data || [];
      console.log("Vitals full data:", records);
      ...
      const latest = (records || [])
        ...
        .sort((a, b) => new Date(b.recorded_at).getTime() - new Date(a.recorded_at).getTime())[0] || records[records.length - 1]; // Fallback to last row if no valid row found
  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on('postgres_changes', { event: '*', schema: 'public', table: 'vitals', filter: `user_id=eq.${user?.id}` }, () => {
        fetchVitals();
      })
      .subscribe();
  ...
  }, [user?.id, period]); // Removed fetchVitals from dependencies
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      const headers = ["Date", "Time", "Heart Rate (bpm)", "Systolic BP (mmHg)", "Diastolic BP (mmHg)", "SpO2 (%)", "Temperature (°C)", "Weight (kg)"];
