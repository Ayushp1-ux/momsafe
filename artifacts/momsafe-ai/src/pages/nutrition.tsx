...
export default function Nutrition() {
  ...
  useEffect(() => {
    const fetchAISuggestions = async () => {
      ...
      const AI_URL =
        "https://frupdhslwhwyxclhcqve.supabase.co/functions/v1/nutrition-";
  ...
  const handleAnalyzeMeal = async () => {
    ...
    try {
      ...
      // Multi-turn logic: If AI returns more questions, stay in Step 2
      if (data.questions && data.questions.length > 0) {
        setQuestions((prev) => [...prev, ...data.questions]);
        toast.info("A few more details needed for accuracy...");
      } else {
        setPreview(data);
        setStep(3);
      }
          ...
          <div className="relative z-10 p-5 flex flex-col h-full">
            ...
            <div className="grid grid-cols-2 gap-2 mt-1">
              ...
              >
                ...
                >
                  {Math.max(0, calorieGoal - totalCalories).toLocaleString(
                    undefined,
                    {
                      minimumFractionDigits: 0,
                      maximumFractionDigits: 1,
            ...
            {step === 2 && (
              <div className="space-y-5 animate-in fade-in slide-in-from-right-4">
                ...
                <button
                  ...
                  disabled={
                    loading || questions.some((q) => !answers[q.key])
                  }
                  className="w-full py-3.5 bg-blue-600 text-white rounded-xl text-sm font-bold flex items-center justify-center gap-2 hover:bg-blue-700 transition-all shadow-lg shadow-blue-200 disabled:bg-gray-300 disabled:shadow-none"
                >
                  {loading ? (
                    <Loader2 className="w-4 h-4 animate-spin" />
                  ) : (
