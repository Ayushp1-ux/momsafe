...
import { Activity, Plus, Heart, Droplets, Thermometer, Scale, ChevronLeft, ChevronRight, Check, AlertCircle, Info, RefreshCcw, Minus } from "lucide-react";
...
export function ManualVitalsInput({ onSuccess }: ManualVitalsInputProps) {
  ...
  const fetchLastVitals = useCallback(async () => {
    ...
    console.log("Vitals full data:", data);
  ...
  const currentStatus = useMemo(() => {
    ...
    if (selectedVital === "blood_pressure") {
      val = Number(formData.systolic_bp);
    } else {
      val = Number(formData[selectedVital as keyof typeof formData]);
    }
    
    // Ensure we only return a status if we have a valid number
            ...
            {isBP ? (
              ...
              <div className="space-y-4">
                ...
                <div className="px-4 py-2">
                  <Slider
                    value={[Number(formData[selectedVital! as keyof typeof formData]) || meta.normalMin]}
                    min={meta.min}
                    max={meta.max}
                    step={meta.step}
                    onValueChange={(v) => {
                      // Prevent updating to the same value to avoid loops
                      const newVal = String(v[0]);
                      if (formData[selectedVital! as keyof typeof formData] !== newVal) {
                        handleChange(selectedVital!, newVal);
                      }
                    }}
                    className="py-4"
                  />
